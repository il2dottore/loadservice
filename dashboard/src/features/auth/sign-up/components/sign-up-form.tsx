...
export function SignUpForm({
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      await signUp.mutateAsync({ ...data, firstName: data.email.split('@')[0], lastName: 'User', username: data.email.split('@')[0] })
      toast.success(`Account created for ${data.email}. Please sign in.`)
      form.reset()
    } catch {
