...
export const rolePermissionEntity = pgTable('roles_permissions', {
  roleKey: varchar('role_key').notNull().references(() => roleEntity.key),
  permissionKey: varchar('permission_key', { length: 255 }).notNull().references(() => permissionEntity.key),
}, (table) => {
  return [
    primaryKey({
      columns: [
...
export const rolesPermissionsRelations = relations(rolePermissionEntity, ({ one }) => ({
  role: one(roleEntity, {
