...
export const networkServerEntity = pgTable('networks_servers', {
  networkId: integer('network_id').notNull().references(() => networkEntity.id, { onDelete: 'cascade' }),
  serverId: integer('server_id').notNull().references(() => serverEntity.id, { onDelete: 'cascade' }),
...
export const networksServersRelations = relations(networkServerEntity, ({ one }) => ({
