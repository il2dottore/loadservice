...
@Injectable()
export class MethodRepository extends BasePostgresRepository<
  typeof methodsTable
> {
  ...
  async findAllWithFeatures() {
    return this.postgres
      ...
      .from(methodsTable)
      .leftJoin(
        methodsFeaturesTable,
        eq(methodsFeaturesTable.methodId, methodsTable.id),
      );
  }

  async assignFeature(methodId: number, featureId: string) {
    const [result] = await this.postgres
      .insert(methodsFeaturesTable)
      .values({ methodId, featureId })
      .onConflictDoNothing()
      .returning();
    return result ?? null;
  }
  ...
  async removeFeature(methodId: number, featureId: string) {
    const [result] = await this.postgres
      .delete(methodsFeaturesTable)
      .where(
        and(
          eq(methodsFeaturesTable.methodId, methodId),
          eq(methodsFeaturesTable.featureId, featureId),
        ),
      )
      .returning();
    return result ?? null;
